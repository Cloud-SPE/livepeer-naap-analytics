...
func TestRuleAttribution001_CapabilitySnapshotsNormalizeAndConverge(t *testing.T) {
	...
	if h.queryInt(t, `SELECT count() FROM naap.capability_snapshots WHERE org = ? AND orch_address = ?`, h.org, wantAddr) != 2 {
		t.Errorf("RULE-ATTRIBUTION-001: expected 2 historical capability snapshots for %s", wantAddr)
	}
	if h.queryInt(t, `SELECT count() FROM naap.serving_latest_orchestrator_state WHERE orch_address = ?`, wantAddr) != 1 {
		t.Errorf("RULE-ATTRIBUTION-001: expected 1 latest orchestrator state row for %s", wantAddr)
	}
	if got := h.queryString(t, `SELECT version FROM naap.serving_latest_orchestrator_state WHERE orch_address = ?`, wantAddr); got != "0.8.0" {
...
func TestRuleAttribution001_LateCapabilitySnapshotRepairsHistoricalAttribution(t *testing.T) {
	...
	if got := h.queryString(t, `SELECT attribution_status FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, key); got != "unresolved" {
	...
	h.insert(t, []rawEvent{{
		...
		Data: fmt.Sprintf(`[{"address":%q,"local_address":"orch","uri":%q,"version":"0.9.0","hardware":[{"pipeline":"text-to-image","model_id":"model-a"}]}]`,
			orchAddr, orchURI),
		IngestedAt: ts.Add(30 * time.Second),
	}})

	if got := h.queryString(t, `SELECT attribution_status FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, key); got != "resolved" {
		t.Errorf("RULE-ATTRIBUTION-001: repaired attribution_status = %q, want resolved", got)
	}
	if got := h.queryString(t, `SELECT ifNull(attributed_orch_address, '') FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, key); got != orchAddr {
...
func TestRuleAttribution001_FailSafeStatesRemainVisibleInServingOutputs(t *testing.T) {
	...
	if got := h.queryString(t, `SELECT attribution_status FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, hwKey); got != "hardware_less" {
		t.Errorf("RULE-ATTRIBUTION-001: hardware-less attribution_status = %q, want hardware_less", got)
	}
	if got := h.queryString(t, `SELECT ifNull(attributed_orch_address, '') FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, hwKey); got != hwAddr {
		t.Errorf("RULE-ATTRIBUTION-001: hardware-less attributed_orch_address = %q, want %q", got, hwAddr)
	}
	if got := h.queryString(t, `SELECT attribution_status FROM naap.serving_status_samples WHERE canonical_session_key = ? LIMIT 1`, hwKey); got != "hardware_less" {
		t.Errorf("RULE-ATTRIBUTION-001: hardware-less serving attribution_status = %q, want hardware_less", got)
	}
	if h.queryInt(t, `SELECT count() FROM naap.v_api_sla_compliance_by_org WHERE org = ? AND orchestrator_address = ? AND gpu_id IS NULL`, h.org, hwAddr) == 0 {
		t.Errorf("RULE-ATTRIBUTION-001: hardware-less session was dropped from SLA output for %s", hwAddr)
	}

	if got := h.queryString(t, `SELECT attribution_status FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, unresolvedKey); got != "unresolved" {
		t.Errorf("RULE-ATTRIBUTION-001: unresolved attribution_status = %q, want unresolved", got)
	}
	if got := h.queryString(t, `SELECT ifNull(orch_address, '') FROM naap.serving_status_samples WHERE canonical_session_key = ? LIMIT 1`, unresolvedKey); got != unresolvedAddr {
		t.Errorf("RULE-ATTRIBUTION-001: unresolved serving orch_address = %q, want %q", got, unresolvedAddr)
	}
}
...
func TestRuleAttribution001_AmbiguousAndStaleRemainExplicit(t *testing.T) {
	...
	h.insert(t, []rawEvent{
		{
			...
			Data: fmt.Sprintf(`[{"address":%q,"local_address":"orch-old","uri":%q,"version":"0.8.0","hardware":[{"pipeline":"text-to-image","model_id":"model-a"}]}]`,
				staleAddr, staleURI),
	...
	if got := h.queryString(t, `SELECT attribution_status FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, staleKey); got != "stale" {
		t.Errorf("RULE-ATTRIBUTION-001: stale attribution_status = %q, want stale", got)
	}
	if got := h.queryString(t, `SELECT attribution_status FROM naap.fact_workflow_sessions WHERE canonical_session_key = ?`, ambiguousKey); got != "ambiguous" {
...
func TestRuleAttribution007_BlankGatewayRateIsComputable(t *testing.T) {
	...
	total := h.queryInt(t, `SELECT count() FROM naap.events WHERE org = ?`, h.org)
	blank := h.queryInt(t, `SELECT countIf(gateway = '') FROM naap.events WHERE org = ?`, h.org)
