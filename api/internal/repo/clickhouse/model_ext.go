...
func (r *Repo) ListModelPerformance(ctx context.Context, p types.QueryParams) ([]types.ModelPerformance, error) {
	...
	rows, err := r.conn.Query(ctx, `
		...
		FROM naap.api_gpu_inventory AS gi
		JOIN naap.api_fps_hourly AS fh
...
func (r *Repo) GetModelDetail(ctx context.Context, modelID string, p types.QueryParams) (*types.ModelDetail, error) {
	...
	if p.Pipeline == "" {
		...
		row := r.conn.QueryRow(ctx, `
			SELECT pipeline FROM naap.api_gpu_inventory
	...
	orchRows, err := r.conn.Query(ctx, `
		...
		FROM naap.api_gpu_inventory AS gi
		LEFT JOIN naap.api_latest_orchestrator_state AS os ON gi.orch_address = os.orch_address
		LEFT JOIN naap.api_fps_hourly AS fh
