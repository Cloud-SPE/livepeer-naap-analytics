...
func (r *Repo) GetDashboardKPI(ctx context.Context, windowHours int) (*types.DashboardKPI, error) {
	...
	var buckets []kpiBucket
	for rows.Next() {
		var b kpiBucket
		if err := rows.Scan(&b.ts, &b.sessions, &b.successes); err != nil {
	...
	hourlySessions := make([]types.DashboardHourlyBucket, 0, len(buckets))
	hourlyUsage := make([]types.DashboardHourlyBucket, 0, len(buckets))
	for _, b := range buckets {
		totalSessions += b.sessions
		totalSuccesses += b.successes
		ts := b.ts.UTC().Format(time.RFC3339)
		hourlySessions = append(hourlySessions, types.DashboardHourlyBucket{Hour: ts, Value: float64(b.sessions)})
		hourlyUsage = append(hourlyUsage, types.DashboardHourlyBucket{Hour: ts, Value: float64(b.successes)})
	}

	successRatePct := divSafe(float64(totalSuccesses), float64(totalSessions)) * 100
	sessionDelta, successRateDelta := computeKPIDeltas(buckets)
	...
	return &types.DashboardKPI{
		...
		DailyUsageMins: types.DashboardMetricDelta{}, // Phase 2: network demand table
...
func computeKPIDeltas(buckets []kpiBucket) (sessionDelta, successRateDelta float64) {
	if len(buckets) < 2 {
		return 0, 0
	}
	mid := len(buckets) / 2
...
// GetDashboardPipelines returns per-pipeline session counts for the last 24 h (R16-2).
// Source: naap.api_stream_hourly.
func (r *Repo) GetDashboardPipelines(ctx context.Context, limit int) ([]types.DashboardPipelineUsage, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&pipeline, &sessions, &successes); err != nil {
		...
		result = append(result, types.DashboardPipelineUsage{
			...
			Mins:     float64(sessions), // approximation until network-demand minutes are available
			AvgFps:   0,                 // Phase 2: agg_stream_status_samples
		})
	}
	if err := rows.Err(); err != nil {
