...
func (s *Server) handleListAIBatchJobs(w http.ResponseWriter, r *http.Request) {
	...
	p := parseTimeWindowParams(r)
	rows, next, err := s.svc.ListAIBatchJobs(r.Context(), p)
	if err != nil {
	...
	respondJSON(w, http.StatusOK, types.CursorEnvelopeAIBatchJobs{
		Data:       rows,
...
func (s *Server) handleListBYOCJobs(w http.ResponseWriter, r *http.Request) {
	...
	p := parseTimeWindowParams(r)
	rows, next, err := s.svc.ListBYOCJobs(r.Context(), p)
	if err != nil {
		if errors.Is(err, types.ErrInvalidCursor) {
			writeError(w, http.StatusBadRequest, "Bad Request", err.Error())
			return
	...
	respondJSON(w, http.StatusOK, types.CursorEnvelopeBYOCJobs{
		Data:       rows,
		Pagination: types.CursorPageInfo{NextCursor: next, HasMore: next != "", PageSize: len(rows)},
		Meta:       buildMeta(r),
	})
