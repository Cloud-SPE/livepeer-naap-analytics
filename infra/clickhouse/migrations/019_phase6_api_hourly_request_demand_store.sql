-- Phase 6.1 of serving-layer-v2: promote api_hourly_request_demand from an
-- on-demand view over canonical_ai_batch_jobs + canonical_byoc_jobs to a
-- resolver-written store matching the _hourly family convention (Phase 1
-- did the same for streaming SLA). Serving read cost drops from "rescan
-- two canonical job feeds every request" to "one MergeTree primary-key
-- lookup plus a latest-slice argMax".
--
