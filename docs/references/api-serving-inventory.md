...
| Demand by model and pipeline, including no-capacity | stream | `api_hourly_streaming_demand` | one row per `(window_start, org, gateway, region, pipeline_id, model_id)` | `requested_sessions`, `startup_success_sessions`, `no_orch_sessions`, `startup_failed_sessions`, `effective_failed_sessions`, `output_fps_sum`, `status_samples`, `total_minutes`, `ticket_face_value_eth` | resolver-published `canonical_streaming_demand_hourly_store`, derived from canonical session state | lineage is `normalized_ai_stream_status` plus `normalized_stream_trace`; this remains the additive hourly streaming demand surface, now published under the `api_hourly_*` family |
| Demand by orchestrator, pipeline, model, and GPU quality surface | stream | `api_hourly_streaming_sla`, `api_hourly_streaming_gpu_metrics` | one row per `(window_start, org, orchestrator_address, pipeline_id, model_id, gpu_id, region)` | `requested_sessions`, `startup_success_sessions`, `effective_failed_sessions`, `output_fps_sum`, `status_samples`, latency support sums and counts | resolver-published SLA and GPU metric stores derived from canonical session state | use `api_hourly_streaming_sla` for unresolved or null-GPU rows and `api_hourly_streaming_gpu_metrics` for hardware-resolved GPU latency or FPS analysis |
| Demand by model, pipeline, GPU, and no-orch breakdown | ai-batch | `api_fact_ai_batch_job` | one row per `(org, request_id)` | `count(*)`, `countIf(selection_outcome = 'selected')`, `countIf(selection_outcome = 'no_orch')`, `countIf(success = 1)`, `sum(duration_ms)`; join `api_fact_ai_batch_llm_request` on `(org, request_id)` for token fields | `canonical_ai_batch_jobs` | lineage is `normalized_ai_batch_job`; there is still no builtin request hourly surface with richer per-job additive fields than `api_hourly_request_demand` |
...
| LLM enrichment | `api_fact_ai_batch_llm_request` | `api_fact_ai_batch_llm_request` joined to `api_fact_ai_batch_job` | none beyond job-level joins today | `api_fact_ai_batch_llm_request.price_per_unit` | keep this one-row-per-request so token joins do not inflate job counts |
