...
`tests/python/scripts/export_scenario_fixtures.py` and publishes raw event envelopes to the
input topic consumed by Flink (`streaming_events` by default).

Supported source records inside fixture JSONL:
- streaming_events (canonical raw envelope rows)
"""
...
def extract_raw_event(record: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if table not in RAW_EVENT_TABLES:
        return None
    # streaming_events rows are canonical raw envelopes; preserve shape and
    # normalize `data` when serialized as a JSON string.
...
def main() -> None:
    ...
    if stats["files_total"] > 0 and stats["files_missing"] == stats["files_total"]:
        raise RuntimeError(
            "All fixture files referenced by manifest are missing. "
            "Re-select/promote a valid snapshot manifest, e.g. "
            "`uv run --project tests/python python tests/python/scripts/select_fixture_manifest.py`."
        )
